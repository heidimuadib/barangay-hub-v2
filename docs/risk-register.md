...
| R-2-03 | Slice 2A ships the registry schema before the public sign-up surface (2B) and the Storage broker (2F); a partial deployment could expose onboarding without anti-enumeration/rate limiting or accept evidence metadata with nowhere safe to put bytes | Subpart sequencing ([roadmap](./IMPLEMENTATION_ROADMAP.md) Slice 2) | L (nothing is deployed; hosted writes prohibited) | M | L | Process | **CLOSED by 2G (2026-08-02).** The condition was a *partial* Slice 2 reaching an environment. It cannot now occur: 2A–2F ship as one branch behind a single PR, and every subpart the risk named is present in it — 2B's anti-enumeration and rate limiting, and 2F's private bucket and signed brokering. Nothing was deployed anywhere in the interim (hosted writes stayed prohibited throughout), so the risk expires unrealised rather than being mitigated. Hosted exposure remains blocked by R-1-04 / R-0B-05 / R-0B-06, which are separate and still open | Slice 2 implementer | closed |
| R-2-04 | A resident cannot reach the verification queue through the browser: onboarding opens a `draft` application and `submit_verification` requires one identity + one residency evidence item, but the upload surface is 2F — so the 2D queue is only reachable for applications submitted by seed or by direct API call | Found while building the 2D e2e suite (2026-08-01); `submit_verification` evidence gate in `20260802030000_registry_functions.sql`; onboarding action opens a draft and stops | — | — | — | Process/UX | **CLOSED by 2F (2026-08-02).** The resident now completes the whole journey in a real browser: pick a synthetic file → signed one-object upload into the private bucket → server-verified finalization → submit. Proven end to end by `apps/web/tests/e2e/evidence.spec.ts` ("uploads both documents and submits entirely through the browser"), which uses no direct database insert, no service-role shortcut and no seeded evidence. The 2D workaround (submitting through the resident's own granted RPCs) is no longer needed for the journey, though the verification suite still uses it to mint fixtures cheaply | Slice 2F implementer | closed |
| R-2-05 | The Slice 1 shell header's navigation links are 20–27px tall, below the 44px touch target this project states as Definition of Done — hardest on exactly the phone users the platform is for | Measured in 2G while building `apps/web/tests/e2e/accessibility.spec.ts` across both viewport projects; pre-existing, not introduced by Slice 2 | H (every authenticated page shows the header) | L (links are reachable and keyboard-operable; the cost is mis-taps, not exclusion) | M | UI/Accessibility | **Recorded, deliberately not fixed here.** The accessibility spec exempts inline nav links explicitly and says why, so the gap is visible in the test rather than silently passing. Fixing it means re-laying-out the shell chrome, which belongs to the US-UI-002 rework already recorded as a Slice 1 deferral — doing it from inside Slice 2 would put an unreviewed global layout change in a registry PR. Every control Slice 2 itself added meets 44px, and one genuine 27px CTA found by the same sweep (the resident dashboard's "View my registration") **was** fixed. **Widened by 3B (2026-08-03):** the resident shell gained two more links of the same kind (**Documents**, **My requests**), so the gap now spans five header links rather than three. **CLOSED by 3D (2026-08-04):** US-UI-002 was assigned to Slice 3, so every navigation link in the resident, staff and platform shells now carries `min-h-11` (17 targets). The accessibility spec's `nav` exemption — which existed to keep the gap visible rather than silently passing — was **removed** in the same change, so shell navigation is held to the same 44px as every other control and a regression fails the sweep on both viewports. Residual: the bottom navigation, notification centre and density controls named in the original deferral are still not built (see the deferral table in `decisions/blockers.md`) | UI slice implementer (US-UI-002) | closed |
