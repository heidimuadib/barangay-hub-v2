...
| R-1-06 | Under a production server (`next start`), the members-page status chip did not always re-render after a status change, though the database mutation committed and was audited | Observed twice during promotion verification against `next start`; dev servers (local + CI e2e) unaffected; pgTAP confirms the mutation itself | M | L (UI staleness only — no authorization effect) | L | UI | Investigate the server-action revalidation path under production caching in Slice 2 hardening; no test was weakened (CI runs the dev server as configured) | Slice 2 implementer | open |
