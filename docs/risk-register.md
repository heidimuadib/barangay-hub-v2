...
| R-1-04 | Auth surfaces rely on Supabase defaults for rate limiting; Option C (ADR-0006) adds a public sign-up surface in Slice 2 | Auth config (Slice 0b); DEC-AUTH-01 ruling points 2/13 | M | M | M | Auth | Now a Slice 2 build requirement: anti-enumeration + app-level rate limiting on sign-up and sign-in, hard-gated before hosted public exposure | Slice 2 implementer | open (activated) |
