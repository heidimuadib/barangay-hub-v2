...
| R-1-03 | No automated gate runs on push until the standalone repository is pushed to its GitHub remote and the workflow's first run passes | DEC-REPO-01 resolved locally (promotion + hooks + valid workflow at root); no remote exists yet | M | M | M | Process | Local promotion complete; Husky enforces commit/push gates locally. Remaining: create `barangay-hub-v2` remote, push, confirm the workflow run — then close | Repository owner | mitigated (partially — pending first push) |
