...
| R-2-02 | Duplicate-person resolution could destroy identity history if implemented as a destructive merge | D2-02 **approved** (ADR-0006): supersede-and-link only, admin capability + reason + audit, no automatic merge | L (design now fixed) | H | M | Data integrity | Ruling bounds the risk; residual is implementation fidelity — pgTAP must prove supersede preserves both records and refuses destructive paths | Slice 2 implementer | mitigated (by ruling; verify in build) |
| R-2-04 | A resident cannot reach the verification queue through the browser: onboarding opens a `draft` application and `submit_verification` requires one identity + one residency evidence item, but the upload surface is 2F — so the 2D queue is only reachable for applications submitted by seed or by direct API call | Found while building the 2D e2e suite (2026-08-01); `submit_verification` evidence gate in `20260802030000_registry_functions.sql`; onboarding action opens a draft and stops | H (certain until 2F ships) | L (no user-facing exposure — the whole flow is pre-deployment) | L | Process/UX | Not a defect: the subpart boundary is the cause, and the 2D e2e suite submits through the resident's OWN granted RPCs on their own token (metadata only, no service-role, no direct table write) so the workflow is genuinely proven now. 2F closes it by delivering the upload UI; the resident status page already tells residents to bring documents to the office in the meantime | Slice 2F implementer | open — closes with 2F |
